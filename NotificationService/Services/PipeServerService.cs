...
namespace NotificationService.Services
{
    ...
    public class PipeServerService
    {
        ...
        private string pipeName_AddNotificationToQueue = "notificationServiceAddNotificationToQueue";
        private readonly ILogger<PipeServerService> _logger;
        private readonly INotificationService _notificationService;
        public PipeServerService(ILogger<PipeServerService> logger, INotificationService notificationService)
        {
        ...
        public void MainOri()
        {
            ...
            for (i = 0; i < numThreads; i++)
            {
                servers[i] = new Thread(ServerThreadOri);
                servers[i].Start();
            }
            Thread.Sleep(250);
            while (i > 0)
        ...
        private void ServerThreadOri(object data)
        {
            NamedPipeServerStream pipeServer =
                new NamedPipeServerStream(pipeName_AddNotificationToQueue, PipeDirection.InOut, numThreads);
